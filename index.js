...
mongoose.connect('mongodb://localhost:27017/qatar');
...
app.post('/login', async (req, res) => {
  ...
  res.send({ message: 'Manager login successful', ...userData, role: 'manager' });
});
...
app.post('/add-company', async (req, res) => {
  ...
  try {
    const newCompany = new Company({ name, location, image, userEmail, userName });
...
// 9️⃣ Get company with categories
...
app.post('/company/:id/update-category', async (req, res) => {
  try {
    const { categoryIndex, month, year, monthlyBudget, monthlyExpense, yearlyBudget, yearlyExpense } = req.body;
    ...
    if (!category.monthly || typeof category.monthly !== 'object') category.monthly = {};
    if (!category.yearly || typeof category.yearly !== 'object') category.yearly = {};
...
// 🔥 11️⃣ Add a new category to a company
